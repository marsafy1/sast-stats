...
class CodeQL(SAST):
    def run(self):
        ...
        REMOVE_OLD_DB_COMMAND = f"rm -rf {database_name}"
        CREATED_DB_COMMAND = (
            f"codeql database create {database_name} --language=python --overwrite"
        )
        ANALYZE_DB_COMMAND = f"codeql database analyze py-sast-db --format={format} --output={self.output_path} python-code-scanning.qls python-lgtm-full.qls python-lgtm.qls python-security-and-quality.qls python-security-experimental.qls python-security-extended.qls --download githubsecuritylab/codeql-python-queries githubsecuritylab/codeql-python-queries:security/CWE-798/HardcodedFrameworkSecrets.ql"

        # 1. Create the DB  2. Run the analyzer
        os.system(
            f"{REMOVE_OLD_DB_COMMAND} && {CREATED_DB_COMMAND} && {ANALYZE_DB_COMMAND}"
        )
